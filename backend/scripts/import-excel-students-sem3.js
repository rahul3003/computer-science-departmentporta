...
async function run() {
  const excelPath = 'c:\\PVL\\-Computer-Science-Department-Portal\\II YEAR.xlsx';
  ...
  try {
    ...
    // 2. Clear Existing Semester 3 Students
    console.log('Clearing existing students in Semester 3...');
    // Delete attendance records for Semester 3 students first to avoid foreign key violations
    const s3Students = await prisma.user.findMany({
      where: {
        role: 'STUDENT',
        semester: 'Semester 3'
      },
    ...
    const s3StudentIds = s3Students.map(s => s.id);
    if (s3StudentIds.length > 0) {
      const deletedAttendance = await prisma.attendance.deleteMany({
        where: {
          studentId: { in: s3StudentIds }
        }
      });
      console.log(`✓ Deleted ${deletedAttendance.count} attendance records for Semester 3 students.`);
    }
    ...
    console.log(`✓ Cleared ${deleteResult.count} existing Semester 3 student users.`);
